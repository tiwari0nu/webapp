pipeline {
  ...
  stage ('Check-Git-Secrets') {
    ...
    stage ('Deploy-To-Tomcat') {
            steps {
           sshagent(['tomcat']) {
                sh 'scp -o StrictHostKeyChecking=no target/*.war ank@127.0.0.1:home/ank/apache-tomcat-9.0.68/webapps/webapp.war'
              }      
           }        
    }
  ...
  stage ('DAST') {
      steps {
        sh 'docker run -t owasp/zap2docker-stable zap-baseline.py -t http://192.168.1.9:9090/webapp/ || true'
